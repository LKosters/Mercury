...
- **macOS uses ad-hoc signing (`mac.identity: "-"`), NOT `null`.** With `identity: null` (or `CSC_IDENTITY_AUTO_DISCOVERY=false`) electron-builder skips signing, leaving Electron's broken `linker-signed` stub whose seal doesn't cover the renamed bundle/resources — `codesign --verify` fails and quarantined downloads show the scary **"Mercury.app is damaged — move to Trash"** block. Ad-hoc signing produces a *valid* signature, downgrading that to the normal "unidentified developer" prompt. So: keep `identity: "-"` and do **not** set `CSC_IDENTITY_AUTO_DISCOVERY=false` in the workflow. (Regressed once — 2026-07-07.)
- **First launch on macOS still needs a bypass** (no notarization): right-click → Open, or System Settings → Privacy & Security → "Open Anyway"; or `xattr -cr /Applications/Mercury.app`. Fully clean first-launch requires an Apple Developer ID cert + notarization ($99/yr) — not wired up. Windows is unsigned (SmartScreen "More info → Run anyway").
- **The in-app updater avoids this** on subsequent updates: `updater.js` strips `com.apple.quarantine` from the downloaded DMG before opening, so only the very first browser download hits the Gatekeeper prompt.
...
### 2026-07-07 — Fix macOS "app is damaged" (ad-hoc signing)
**Problem:** the v0.1.0 mac build opened to *"Mercury.app is damaged and can't be opened"*. Cause: `mac.identity: null` + `CSC_IDENTITY_AUTO_DISCOVERY: false` made electron-builder skip signing, so the renamed bundle kept Electron's invalid linker-signed stub (`codesign --verify` failed) — which reads as "damaged" once quarantined.
**Fix:** set `mac.identity: "-"` (valid ad-hoc signature) and removed the `CSC_IDENTITY_AUTO_DISCOVERY` env from the workflow. Downgrades the block to the bypassable "unidentified developer" prompt. Notarization still not wired up.
